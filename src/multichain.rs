use crate::{Asset, AssetCollector, MultichainData, impls::RPCCalls};
...
const ASSOCIATED_ASSETS_STORAGE_KEY: &[u8] = b"collateral-reader::multichain-associated-assets-store";
...
fn get_chains() -> Result<Vec<Chain>, Error> {
  let multichain_chains_store: StorageValueRef = 
    StorageValueRef::persistent(CHAINS_STORAGE_KEY);
...
fn get_assets() -> Result<Vec<Asset>, Error> {
  ...
  let multichain_assets_store: StorageValueRef =
    StorageValueRef::persistent(ASSETS_STORAGE_KEY);
  if let Ok(Some(assets)) = multichain_assets_store.get::<Vec<Asset>>() {
    ...
    // Deserializing JSON to struct, thanks to `serde` and `serde_derive`
    let response: BTreeMap<String, MultichainToken> =
    serde_json::from_slice(&resp_str).map_err(|e| {
    ...
    let whitelisted_asset_symbols: Vec<&str> = vec!["ETH", "WETH", "WBTC", "USDC", "USDT", "DAI"];
    for (_, value) in response.iter() {
      ...
      for (_, associated_asset_dest_chains) in value.dest_chains.iter() {
        for (_, associated_asset) in associated_asset_dest_chains.iter() {
          if associated_asset.anytoken.is_none() || associated_asset.fromanytoken.is_none() {
          ...
          associated_assets.push(AssociatedAsset {
            ...
            address_that_has_locked_assets: associated_asset.anytoken.clone().unwrap().address.clone().into(),
            address_that_has_issued_assets: fromanytoken.address.clone().into(),
            minted_asset: fromanytoken.chain_id == Some(String::from(str::from_utf8(ETH_CHAIN_ID).unwrap())),
          })
        }
      }
      assets.push(asset);
...
fn get_erc20_token_balance(url: &str, method: String, params: (BalanceParams, String)) -> Result<Vec<u8>, Error> {
  ...
  let request_body_str = serde_json::to_string(&request_body).map_err(|_| <Error>::GetAssetsError)?;
  let request = Request::post(url, vec![request_body_str.clone()])
    .add_header("content-type", "application/json");

  // Keeping the offchain worker execution time reasonable, so limiting the call to be within 3s.
  let timeout = sp_io::offchain::timestamp()
    .add(Duration::from_millis(RPC_FETCH_TIMEOUT_PERIOD));
...
fn get_assets_stats() -> Result<(), Error> {
  let multichain_assets_store: StorageValueRef =
    StorageValueRef::persistent(ASSETS_STORAGE_KEY);
  let assets = multichain_assets_store
    .get::<Vec<Asset>>()
    .map_err(|e| {
  ...
  let multichain_chains_store: StorageValueRef =
    StorageValueRef::persistent(CHAINS_STORAGE_KEY);
  let chains = multichain_chains_store
    .get::<Vec<Chain>>()
    .map_err(|e| {
      log::error!("chains {:?}", e);
      <Error>::GetAssetsStatsError
  ...
  let associated_assets = 
    multichain_associated_assets_store
  ...
  for asset in assets.unwrap().iter() {
    let associated_assets_of_current_asset: Vec<AssociatedAsset> = 
      associated_assets
      .clone()
      .unwrap()
      .iter()
    ...
    for associated_asset in associated_assets_of_current_asset.iter() {
      let chain = unwrapped_chains
        .iter()
        .find(|x| x.id == associated_asset.chain_id);
      if chain.is_none() {
        continue;
      }
      let chain_rpc_url = chain.unwrap().rpc.clone();
      let eth_chain = unwrapped_chains
        .iter()
        .find(|x| x.id == ETH_CHAIN_ID.to_vec())
        .unwrap();
    ...
    if  tla > 0 && tia > 0 {
      ...
      let asset_prev_stats = unwrapped_prev_stats.iter().find(|i| i.asset_id == asset.symbol.clone());
...
fn get_total_amount(address_map: BTreeMap<Vec<u8>, (Vec<u8>, AssetId)>) -> u128 {
  ...
  for (router_address, (chain_rpc_url, asset_address)) in address_map.iter() {
    ...
    match result {
      Ok(res) => { 
        let parsed = serde_json::from_slice(&res)
          .map_err(|e| {
        ...
        let parsed_amount = u128::from_str_radix(crop_letters(&amount.result, 2), 16).unwrap();
...
impl RPCCalls for MultichainRPCHelper {
  fn get_supported_assets(&self) -> Result<Vec<Asset>, &'static str> {
    ...
    let multichain_stats_store: StorageValueRef = 
      StorageValueRef::persistent(STATS_STORAGE_KEY);
    let stats = multichain_stats_store.get::<Vec<MultichainAssetStats>>()
      .map_err(|e| {
        log::error!("multichain_stats_store {:?}", e);
    ...
    let long_lived_stats = stats
      .unwrap()
      .clone();
    let asset_stats = long_lived_stats
      .iter()
    ...
    let multichain_associated_assets_store: StorageValueRef = 
      StorageValueRef::persistent(ASSOCIATED_ASSETS_STORAGE_KEY);
    let associated_assets = multichain_associated_assets_store
      .get::<Vec<AssociatedAsset>>()
      .map_err(|e| {
        log::error!("multichain_associated_assets_store {:?}", e);
    ...
    let long_lived_associated_assets = unwrapped_associated_assets
      .clone()
      .unwrap();
    ...
    let all_associated_assets = multichain_associated_assets_store
      ...
      .unwrap()
      .unwrap();
    let associated_asset = all_associated_assets
      .iter()
      .find(|i| i.asset_id == minted_asset);
...
impl AssetCollector for MultichainData {
    ...
    let helpers: Vec<Box<dyn RPCCalls>> =
      vec![Box::new(MultichainRPCHelper {})];
