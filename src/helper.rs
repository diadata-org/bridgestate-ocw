pub mod Helper {
    ...
    use frame_support::{Twox128, StorageHasher, Twox64Concat};
    use sp_runtime::{
        offchain::{
            http,
            Duration,
...
struct RpcResponse {
    ...
    pub fn generate_storage_key(module_prefix: &str, storage_item_prefix: &str) -> Vec<u8> {
        ...
        let mut storage_key: Vec<_> = Vec::with_capacity(
            module_hash.len() + storage_hash.len(),
        );
    ...
    pub fn to_hex(storage_key: Vec<u8>) ->String{
        let hex_string: String = storage_key.iter()
    ...
    pub fn generate_double_storage_key(module_prefix: &str, storage_item_prefix: &str, id: &str) -> Vec<u8> {
    ...
    pub fn generate_double_storage_keys(module_prefix: &str, storage_item_prefix: &str, key1: &str, key2: &str) -> Vec<u8> {
        let mut storage_key = Vec::new();
        let module_hash = Twox128::hash(module_prefix.as_bytes());
        let storage_hash = Twox128::hash(storage_item_prefix.as_bytes());
    ...
    pub fn fetch_data(method: &str, param: &str) -> Result<Vec<u8>, http::Error> {
        ...
        rpc_request = rpc_request.replace(
            "passparam",
            param,
        );
    ...
    pub fn total_user_vault_collateral(token: &str) -> u128 {
     ...
     if token.ne(&String::from("DOT")){
		storage_key_hash = "0x".to_owned() + &storage_key_hash + "ed11b90b07067c86130c95aabfcb699c01020000000001";

	 } else{
		storage_key_hash = "0x".to_owned() + &storage_key_hash + "d6bfa4fbbbb302d0f4e13a890467318100000001";
	...
	pub fn oracle(token: &str) -> u128 {
     ...
     if token.ne(&String::from("DOT")){
		storage_key_hash = "0x".to_owned() + &storage_key_hash + "e8ee4335018f6743c682ee73dfe0674c000102000000";

	 } else{
		storage_key_hash = "0x".to_owned() + &storage_key_hash + "7b79be5e9b370ba6d080e4e2af7b7b89000000";
